...
const LibraryModule = {
  ...
  getDefaults() {
    return {
      podcast: [
        { id:'p1', name:'思文败类', episode:'EP.68 爱自己到底是什么', timestamp:'12:30-13:15', quote:'执行力差其实是太惯着自己了，真正的爱自己是尊重内心想变好的渴望', emotion:'醍醐灌顶', tags:['执行力','自我成长'], used:true },
        { id:'p2', name:'岩中花述', episode:'EP.45 母女之间那堵墙', timestamp:'22:00-23:00', quote:'那个给他过生日，不就是你一直期待的妈妈爱你的方式吗', emotion:'扎心', tags:['东亚家庭','母女关系'], used:true },
        { id:'p3', name:'天真不天真', episode:'EP.112 别把努力当回事', timestamp:'08:45-09:30', quote:'努力是最不值钱的东西，努力做出贡献才值钱', emotion:'醍醐灌顶', tags:['职场','认知'], used:false },
        { id:'p4', name:'自我进化论', episode:'EP.37 停止内耗的唯一方法', timestamp:'15:00-16:00', quote:'你大部分的痛苦，不是因为做得不好，而是因为想得太多', emotion:'治愈', tags:['内耗','情绪'], used:false },
        { id:'p5', name:'Steve说', episode:'EP.203 原生家庭不是你的错', timestamp:'28:00-29:00', quote:'你可以理解父母，但不必承接他们的情绪', emotion:'释怀', tags:['原生家庭','边界感'], used:false },
        { id:'p6', name:'来都来了', episode:'EP.89 打工而已别上头', timestamp:'10:00-10:45', quote:'一份工作而已，别让它定义你的人生价值', emotion:'通透', tags:['职场','心态'], used:false },
      ],
      images: [
        { id:'img1', name:'黄昏城市剪影', style:'氛围感/孤独', scene:'情绪共鸣类', source:'unsplash', path:'/素材/黄昏城市.jpg' },
        { id:'img2', name:'安静的书桌', style:'治愈/温暖', scene:'自我成长类', source:'unsplash', path:'/素材/书桌.jpg' },
        { id:'img3', name:'窗边剪影', style:'思考/安静', scene:'认知升级类', source:'pinterest', path:'/素材/窗边.jpg' },
        { id:'img4', name:'极简几何', style:'现代/理性', scene:'职场类', source:'canva', path:'/素材/几何.jpg' },
      ],
      music: [
        { id:'m1', name:'Gymnopédie No.1', artist:'Erik Satie', style:'钢琴/忧伤', emotion:'治愈', source:'网易云', link:'搜索 Gymnopédie No.1' },
        { id:'m2', name:'Saman', artist:'Ólafur Arnalds', style:'极简/钢琴弦乐', emotion:'思考', source:'网易云', link:'搜索 Ólafur Arnalds Saman' },
        { id:'m3', name:'River Flows in You', artist:'Yiruma', style:'钢琴/温暖', emotion:'治愈', source:'QQ音乐', link:'搜索 River Flows in You' },
        { id:'m4', name:'Comptine', artist:'Yann Tiersen', style:'钢琴/叙事', emotion:'怀旧', source:'网易云', link:'搜索 Comptine' },
        { id:'m5', name:'剪映-安静的夜', artist:'剪映音乐库', style:'轻音乐/安静', emotion:'安静', source:'剪映', link:'剪映音乐库搜索"安静的夜"' },
      ],
      topics: [
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:14px">
          ${items.map(img => `
            <div class="card" style="text-align:center">
              ...
              <div style="font-size:11px;color:var(--text-muted);margin-top:4px">来源: ${img.source}</div>
            </div>
          `).join('')}
        </div>
      `;
  ...
  showAddModal() {
    ...
    if (tab === 'podcast') {
      ...
      fields = `
        ...
        <div class="form-group"><label>来源</label><input class="input" id="lib-source" placeholder="如：unsplash"></div>
      `;
    } else if (tab === 'music') {
      fields = `
        ...
        <div class="form-group"><label>来源/链接</label><input class="input" id="lib-link" placeholder="如：网易云搜索 xxx"></div>
      `;
    } else if (tab === 'topics') {
      fields = `
  ...
  addItem() {
    ...
    if (tab === 'podcast') {
      ...
      App.data.library.images.push({ id:'img'+Date.now(), name:getName(), style:document.getElementById('lib-style')?.value.trim()||'', scene:document.getElementById('lib-scene')?.value.trim()||'', source:document.getElementById('lib-source')?.value.trim()||'' });
    } else if (tab === 'music') {
      App.data.library.music = App.data.library.music || [];
      App.data.library.music.push({ id:'m'+Date.now(), name:getName(), artist:document.getElementById('lib-artist')?.value.trim()||'', style:document.getElementById('lib-style')?.value.trim()||'', emotion:document.getElementById('lib-emotion')?.value.trim()||'', link:document.getElementById('lib-link')?.value.trim()||'' });
    } else if (tab === 'topics') {
